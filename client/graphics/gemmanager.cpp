...
int GraphicGemManager::Generate(int id, int x, int y, int type, int fallen_pixel) {
    ...
    stones_[id] = Gem(coordinate_x, coordinate_start_y, 0, 0, type);
    stones_[id].set_rotating_speed(Gem::kRotatingSpeed);
...
int GraphicGemManager::SetRotate(int id, int rotateMode) {
    ...
    switch (rotateMode) {
        ...
        case kRotateInverse: {
            stones_[id].set_rotating_speed(-1.f * Gem::kRotatingSpeed);
            break;
        }
...
int GraphicGemManager::SetPause(bool is_pause) {
    ...
    std::shuffle(will_set_pause.begin(), will_set_pause.end(), std::default_random_engine(std::random_device{}()));
    ...
    while (!is_falling && !animation_queue_.empty()) {
        if (animation_queue_.front()->type() == AnimationFactory::kAnimationEmpty) {
            ...
            while (!animation_queue_.empty() && animation_queue_.front()->type() == AnimationFactory::kAnimationFall) {
                ...
                stones_[id].set_falling(Gem::kFallingSpeed, animation->tag());
            }
            is_falling = true;
            break;
            ...
            if (animation_queue_.front()->tag() == 0) {
                stones_[id1].set_swaping(stones_[id2].x(), stones_[id2].y(), Gem::kSwapingSpeed);
                stones_[id2].set_swaping(stones_[id1].x(), stones_[id1].y(), Gem::kSwapingSpeed);
            ...
            if (animation_queue_.front()->tag() == 1) {
                stones_[id].set_removing(Gem::kRemovingSpeed, Gem::kRemovingAcceleration);
            } else {
